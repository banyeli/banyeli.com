...
export async function middleware(request: NextRequest) {
  ...
  if (!user?.email || !allowedEmails().has(user.email.toLowerCase())) return NextResponse.redirect(new URL("/login", request.url));
  return response;
}

export const config = { matcher: ["/((?!login|auth/callback|_next/static|_next/image|favicon.ico).*)"] };
